...
Using R to review production data, insights are gathered to help AutoRUs manufacturing team address troubles which are hampering production of the newest prototype, the MechaCar. Linear regression analysis, statistical summaries, and t-tests help assess the manufacturing team's situation and provide feedback on production developments to implement for higher quality results. 
...
At first glance, the summary for all the lots combined looks favorable. The variance of the suspension coils is 62.29 psi which is less than the 100 psi design specifications for the MechaCar suspension coils. However, if the table is broken down into individual lot categories as shown below, it becomes apparent that Lot 3 exceeds the design specifications with a variance of 170.29 psi. While Lot 1 and Lot 2 meet the current manufacturing design specifications, Lot 3 does not and requires further inspection.
...
A one sample t-test is performed on the entire manufacturing lots dataset. The resulting p-value is 0.06; we fail to reject the null hypothesis as the difference between sample mean and the population mean is not statistically significant.
